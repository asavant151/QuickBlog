...
const Header = () => {
  ...
  return (
    <div className='sm:mx-16 xl:mx-24 relative'>
      <div className='text-center mt-20 mb-8'>
        ...
        <form onSubmit={onSubmitHandler} className='flex justify-between max-w-lg max-sm:scale-75 mx-auto border border-gray-300 bg-white rounded overflow-hidden'>
            ...
            {!input ? <button type='submit' className='bg-primary text-white px-8 py-2 m-1.5 rounded hover:scale-105 transition-all cursor-pointer'>Search</button> : <button onClick={onClear} type='submit' className='bg-red-500 hover:bg-red-600 text-nowrap text-white px-8 py-2 m-1.5 rounded hover:scale-105 transition-all cursor-pointer'>Clear Search</button>}
        </form>
      </div>
      <img src={assets.gradientBackground} alt="gradientBackground" className='absolute -top-50 opacity-50 -z-1' />
    </div>
  )
}
