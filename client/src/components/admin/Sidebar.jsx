...
const Sidebar = () => {
  return (
    <div className='flex flex-col border-r border-gray-200 min-h-full pt-6'>
      ...
      <NavLink to="/admin/messages" className={({isActive}) => `flex items-center gap-3 py-3.5 md:px-9 md:min-w-64 cursor-pointer ${isActive && "bg-primary/10 border-r-4 border-primary"}`}>
        <img src={assets.email_icon} alt="home" className='min-w-4 w-5' />
        <p className='hidden md:inline-block'>Messages</p>
      </NavLink>
      <NavLink to="/admin/subscribers" className={({isActive}) => `flex items-center gap-3 py-3.5 md:px-9 md:min-w-64 cursor-pointer ${isActive && "bg-primary/10 border-r-4 border-primary"}`}>
        <img src={assets.user_icon} alt="subscribers" className='min-w-4 w-5' />
